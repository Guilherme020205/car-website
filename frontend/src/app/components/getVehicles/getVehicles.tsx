...
export default function Vehicles() {
  ...
  const [valueChangeMinOrMaxVehicle, setValueChangeMinOrMaxVehicle] =
    useState("20");

  const [maxVehicles, setMaxVehicles] = useState(
    Number(valueChangeMinOrMaxVehicle)
  );
      ...
      <div>
        <div ref={divBaseRef} className="mb-4 select-none">
          ...
          >
            {maxItensPage.map((maxItens) => (
              <SelectItem key={maxItens.id} value={maxItens.label}>
                {maxItens.label}
              </SelectItem> // Passando o value correto
            ))}
          </Select>
        </div>
        ...
        <div className="flex flex-row justify-center gap-10 my-20 select-none">
          <button
            ...
            className={`px-4 py-2 hover:bg-red-100 rounded border-red-700 border-[1px] ${
              minVehicles === 0 || filteredVehicles.length === 0
                ? "invisible"
                : "visible"
            }`}
          >
          ...
          <button
            ...
            disabled={
              maxVehicles >= filteredVehicles.length ||
              filteredVehicles.length === 0
            }
            className={`px-4 py-2 hover:bg-green-100 rounded border-green-700 border-[1px] ${
              maxVehicles >= filteredVehicles.length ||
              filteredVehicles.length === 0
                ? "invisible"
                : "visible"
